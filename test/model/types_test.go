...
func TestJSONB_ScanAndValue(t *testing.T) {
	...
	t.Run("valid bytes", func(t *testing.T) {
		...
		err := j.Scan(input)
		assert.NoError(t, err)
		assert.Equal(t, model.JSONB(input), j)
...
func TestFloat32Vector_ScanAndValue(t *testing.T) {
	tests := []struct {
		name          string
		scanInput     interface{}
		expectedScan  model.Float32Vector
		...
		},
		{
			...
			expectedScan:  model.Float32Vector{0.1, 0.2, 0.3},
			expectedValue: "[0.1,0.2,0.3]",
		},
		{
			name:          "valid vector bytes",
			scanInput:     []byte("[1.5,-2.5]"),
			expectedScan:  model.Float32Vector{1.5, -2.5},
