...
func main() {
	...
	sqlFileFlag := flag.String("file", "", "Path to the SQL script file to run")
	flag.Parse()

	if !*migrateFlag && *sqlFileFlag == "" {
	...
	if *sqlFileFlag != "" {
		...
		if err := LoadDevEnv(db, filePath); err != nil {
			log.Fatalf("Failed to run seed SQL script: %v", err)
		}
